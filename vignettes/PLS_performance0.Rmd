...
Now, it is possible to project the individuals on the best components determined before. We can notice that theses components choice differ strongly according to the use of $Q_2$ (1 or 2 components) or $MSEP$ (7 or 8 components). 
...
plot.indiv <- function(object, compX = c(1,2), compY = NULL){
  ...
  graphX <- ggplot(mat.t, aes(t1,t2, colour = colour)) + geom_point() + 
...
The following code lines allow to make two plots : one is the projection on the two first latent variables and the other is the projection on the two last.
