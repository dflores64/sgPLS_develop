---
title: "Q2 indicator for choosing the number of components in PLS model assessment"
...
The $Q^2$ is a assessment indicator for PLS models; for each new component $h$, a new matrix $Y^{(h)}$ is obtained by deflation and compared to the corresponding prediction matrix $\hat{Y}^{(h)}$. The $Q^2$ therefore takes this comparison into account. A $Q^2$ value close to $1$ indicates a good performance. To compute this figure, we must compute two more indicators : the $RSS$ and the $PRESS$.
...
The function below allow to create theses two first datasets.
...
Let's notice that, on average, the response $Y$ is a linear combination from the predictors $X$. Indeed, the function include a matrix product $Y = XU + E$ with $U$ the weight matrix. This condition is important in order to have a good performance of the model.
