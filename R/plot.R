...
plot.indiv0 <- function(object, compX = 1, compY = 1){
  
  ncomp <- object$ncomp
  mat.t <- object$variates$X
  mat.s <- object$variates$Y
  
  # check arguments
  if(compX < 1 || compY < 1){stop("compX and compY must be strictly positive.")}
  if(compX > ncomp || compY > ncomp){stop("compX and compY mustn't exceed ncomp.")}
  
  t <- mat.t[,compX]
  s <- mat.s[,compY]
  plot(t,s,main = "Individual projected over latent variables", 
       xlim = c(min(t),max(t)), ylim = c(min(s),max(s)),
       xlab = "First latent variable in X", ylab = "First latent variable in Y")
}

d <- data.create()
X <- d$X
Y <- d$Y
modele <- PLS(X,Y,ncomp = 2,mode = "regression")
plot.indiv(object = modele)
