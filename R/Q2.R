...
## second version : proposition de chatGPT ----------------------------

q2.pls.bis <- function(X, Y, mode = "regression", ncomp.max = 10) {
  X <- as.matrix(X)
  Y <- as.matrix(Y)
  p <- ncol(X)
  q <- ncol(Y)
  ...
  if (mode == "regression") {
    Y.mean <- matrix(rep(colMeans(Y), each = n), nrow = n)
    RSS0 <- sum((Y - Y.mean)^2)
    model.all <- pls(X, Y, ncomp = ncomp.max, mode = "regression")
    
    for (h in 1:ncomp.max) {
      Y.hat.rss <- matrix(NA, nrow = n, ncol = q)
      Y.hat.press <- matrix(NA, nrow = n, ncol = q)
      
      for (i in 1:n) {
        model <- pls(X[-i,], Y[-i,], ncomp = h, mode = "regression")
        Y.hat.rss[i, ] <- predict(model.all, newdata = X[i,,drop=FALSE])$predict[,,h]
        Y.hat.press[i, ] <- predict(model, newdata = X[i,,drop=FALSE])$predict[,,h]
      }
      
      RSS <- sum((Y - Y.hat.rss)^2)
      PRESS <- sum((Y - Y.hat.press)^2)
      q2[h] <- 1 - PRESS / RSS
    }
    ...
    ncomp.max <- ncol(Y)
    X.mean <- matrix(rep(colMeans(X), each = n), nrow = n)
    RSS0 <- sum((X - X.mean)^2)
    
    model.all <- pls(X, Y, ncomp = ncomp.max, mode = "canonical")
    T.all <- model.all$variates$X
    C.all <- model.all$mat.c
    
    for (h in 1:ncomp.max) {
      X.hat.rss <- matrix(NA, nrow = n, ncol = p)
      X.hat.press <- matrix(NA, nrow = n, ncol = p)
      
      for (i in 1:n) {
        model <- pls(X[-i,], Y[-i,], ncomp = h, mode = "canonical")
        T_i <- model$variates$X
        C_i <- model$mat.c
        
        # Prédiction Leave-One-Out
        X.hat.press[i, ] <- T_i[nrow(T_i), h] * C_i[, h]
        
        # Prédiction globale (RSS)
        X.hat.rss[i, ] <- T.all[i, h] * C.all[, h]
      }
      
      RSS <- sum((X - X.hat.rss)^2)
      PRESS <- sum((X - X.hat.press)^2)
      q2[h] <- 1 - PRESS / RSS
    }
    ...
    stop("Mode must be either 'regression' or 'canonical'.")
  }
  
  # Détermination du nombre optimal de composantes
  lim <- 0.0975
  h.best <- if (any(q2 > lim)) min(which(q2 > lim)) else NA
  
  # Affichage
  plot(q2, type = "b", col = "blue", pch = 16,
       main = "Q² selon le nombre de composantes",
       xlab = "Nombre de composantes", ylab = "Q²")
  abline(h = lim, col = "red", lty = 2)
  
  suggestion <- if (!is.na(h.best)) paste("Nombre optimal de composantes : H =", h.best) else "Aucune composante prédictive"
  
  return(list(q2 = q2, h.best = h.best, suggestion = suggestion))
}
...
q2.pls.ter <- function(X,Y, mode = "regression", ncomp.max = 10){
  X <- as.matrix(X)
  Y <- as.matrix(Y)
  p <- ncol(X)
  q <- ncol(Y)
  ...
  if(mode == "regression"){ # <<<<<<<<<< CASE OF REGRESSION MODE >>>>>>>>>>>>>
    ...
    model.all <- PLS(X = X, Y = Y, ncomp = ncomp.max, mode = mode)
    mat.t.all <- as.matrix(model.all$variates$X)
    mat.d.all <- as.matrix(model.all$mat.d)
    ...
    for(h in 1:ncomp.max){
      ...
      for(i in 1:n){
        ...
        # predictions on the ith individual
        X.hat.rss[i,] <- predict.PLS.bis(model.all, newdata = X[i,])$predict[,,max(h-1,1)]
        X.hat.press[i,] <- predict.PLS.bis(model, newdata = X[i,])$predict[,,h]
